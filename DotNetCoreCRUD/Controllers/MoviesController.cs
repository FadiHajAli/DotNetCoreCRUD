...
namespace DotNetCoreCRUD.Controllers
{
    public class MoviesController : Controller
    {
        ...
        {
            var viewModel = new MoviesFromViewModel
            {
        ...
        public async Task<IActionResult> Create(MoviesFromViewModel model)
        {
            ...
            {
                model.Genres=await _context.Genres.OrderBy(m => m.Name).ToListAsync();
                ModelState.AddModelError("Poster", "Poster cannot be more htan 1MB");
                return View("MovieForm", model);
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult>Edit(MoviesFromViewModel model)
        {
            if(!ModelState.IsValid)
            {
                model.Genres = await _context.Genres.OrderBy(m => m.Name).ToListAsync();
